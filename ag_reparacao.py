...
for g in range(1, geracoes + 1, 1):
    ...
    # repara os individuos infactiveis
    peso_total = np.zeros(npop, dtype=int)
    valor_total = np.zeros(npop, dtype=int)
    for j in range(npop):
        for i in range(n_obj):
        ...
        # algoritmo que repara
        while peso_total[j] > C:
            # procura os itens de menor valor, caso empate o maior peso
            valor_compara = np.zeros(n_obj, dtype=int)
            for k in range(n_obj):
                valor_compara[k] = valor_unidade[k] * filho[j][k]
            #print("j: " + str(j))
            #print("filho1: " + str(filho))
            #print("compara: " + str(valor_compara))
            valor_min = np.min(valor_compara[np.nonzero(valor_compara)])
            ind_valor_min = np.where(valor_compara == valor_min)
            #print("ind_valor_min: " + str(ind_valor_min[0]))
            peso_valor_min = np.zeros(len(ind_valor_min[0]), dtype=int)
            for k in range(len(ind_valor_min[0])):
                peso_valor_min[k] = peso_unidade[ind_valor_min[0][k]]
            #print("pesos" + str(peso_valor_min))
            ind_peso_max = np.argmax(peso_valor_min)
            #print("ind peso: " + str(ind_peso_max))
            filho[j][ind_valor_min[0][ind_peso_max]] = 0
            #print("filho2: " + str(filho))
            peso_total = np.zeros(npop, dtype=int)
            valor_total = np.zeros(npop, dtype=int)
            for k in range(n_obj):
                peso_total[j] = peso_total[j] + filho[j][k] * peso_unidade[k]
                valor_total[j] = valor_total[j] + filho[j][k] * valor_unidade[k]
